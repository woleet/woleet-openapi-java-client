...
public class ReceiptVerificationStatus   {
   ...
   * @return code
  **/
  @ApiModelProperty(example = "null", value = "Proof receipt verification status code:<br> - VERIFIED: the proof receipt is verified (ie. the receipt is valid, the associated Bitcoin transaction has been found in the blockchain and the receipt's `merkle_root` property matches the transaction's `OP_RETURN` field). The time of the Bitcoin block into which the anchoring process occured is returned in the `timestamp` property.<br> Any data whose SHA256 hash matches the proof receipt's `target_hash` is proven to be existant at that time and unmodified.<br> - INVALID_TARGET_PROOF: the receipt's `target_proof` property is not a valid Merkle proof<br> - TX_NOT_FOUND: the receipt's `tx_id` property does not point to a valid Bitcoin transaction<br> - TX_MISMATCH_RECEIPT: the receipt's `merkle_root` property does not match the transaction's `OP_RETURN` field<br> ")
   ...
   * @return text
  **/
  @ApiModelProperty(example = "null", value = "Proof receipt verification status text giving more insight about verification errors. ")
  ...
  public ReceiptVerificationStatus timestamp(Long timestamp) {
   ...
   /**
   * Timestamp of the Bitcoin block into which the anchoring transaction was included (in milliseconds since Unix epoch).
   * @return timestamp
  **/
  @ApiModelProperty(example = "null", value = "Timestamp of the Bitcoin block into which the anchoring transaction was included (in milliseconds since Unix epoch).")
  ...
  @Override
  public int hashCode() {
    return Objects.hash(code, text, timestamp);
  }
